...
interface ServiceFormProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (service: Omit<Service, 'id' | 'performance'>) => void;
  service?: Service;
}
...
export const ServiceForm: React.FC<ServiceFormProps> = ({
  ...
  const validateForm = () => {
    ...
    if (formData.description.length > 250) newErrors.description = 'Description must be 250 characters or less';
    if (formData.details.some(detail => !detail.trim())) newErrors.details = 'All service details must be filled out';
              ...
              <form onSubmit={handleSubmit} className="mt-6 space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Service Image
                    {errors.image && <span className="ml-2 text-xs text-red-500">{errors.image}</span>}
                  </label>
                  <div 
                ...
                <div>
                  <label htmlFor="description" className="block text-sm font-medium text-gray-700">
                    Description *
                    {errors.description && <span className="ml-2 text-xs text-red-500">{errors.description}</span>}
                  </label>
                  <textarea
                    id="description"
                    rows={3}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">
                    Service Details *
                    {errors.details && <span className="ml-2 text-xs text-red-500">{errors.details}</span>}
                  </label>
                ...
                <div>
                  ...
                  <div className="flex space-x-4">
                    <button
                      type="button"
                      onClick={() => setFormData(prev => ({ ...prev, ctaType: 'schedule' }))}
                    ...
                    <button
                      type="button"
                      onClick={() => setFormData(prev => ({ ...prev, ctaType: 'callback' }))}
                      className={`flex-1 px-4 py-3 text-sm font-medium rounded-lg transition-colors ${
