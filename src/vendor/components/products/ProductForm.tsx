...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  useEffect(()=>{
    ...
    const fetchBrands = async () => {
      try {
        const response = await axiosInstance.get(`/sub-category?category=${selectedCategory}`);
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          const response = await axiosInstance.post("/inventory/bulk-create", formData, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
          });  
          alert(`Success: ${response.data.message || "CSV uploaded successfully!"}`);
          window.location.reload();
        } catch (error: any) {
          console.error("Upload error:", error);
          alert(`Error: ${error.response?.data?.message || error.message || "Failed to upload CSV"}`);
        }
      };
