...
import { ShoppingBag, ChevronDown, ChevronUp, CheckSquare } from 'lucide-react';
...
export const ProductSelections: React.FC = () => {
  ...
  useEffect(() => {
    if (activeCategory) {
      ...
      if (category && category.subcategories.length > 0) {
        ...
        if (category.subcategories[0].products && category.subcategories[0].products.length > 0) {
  ...
  const getAvailableSubcategories = () => {
    ...
    return storeStructure.find(cat => cat.name === activeCategory)?.subcategories || [];
  };
  ...
  const getDisplayProducts = () => {
    ...
    if (!activeCategory) {
      ...
      const selectedProductItems = products.filter(product => selectedProducts.includes(product.id));
      return selectedProductItems.reduce((acc, product) => {
        ...
        if (product.isPremium || product.isContractorChoice || product.sponsored ) {
    ...
    let filteredProducts = products.filter(product => product.category == activeCategory);
  ...
  console.log('fitler 2',filteredProducts,activeSubcategory)
    ...
    const subcategoryName = storeStructure
      .find(cat => cat.name === activeCategory)
      ?.subcategories.find(sub => sub.id === activeSubcategory)?.name || activeCategory;
    
    return {
      [subcategoryName]: {
        premium: filteredProducts.filter(p => p.isPremium || p.isContractorChoice || p.sponsored),
        regular: filteredProducts.filter(p => !p.isPremium && !p.isContractorChoice && !p.sponsored)
      }
    };
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="flex items-center gap-2">
                ...
                {storeStructure.map(category => (
                  <button
                    ...
                    className={`w-full px-4 py-2 text-sm text-left hover:bg-gray-50 transition-colors ${
                      activeCategory === category.name ? 'bg-blue-50 text-blue-800' : 'text-gray-700'
                    }`}
                  >
                    {category.name}
                  </button>
          ...
          {activeCategory && (
            <div className="relative">
              ...
              >
                {activeSubcategory 
                  ? getAvailableSubcategories().find(sub => sub.id === activeSubcategory)?.name 
                  : 'Select Subcategory'}
                ...
                <div className="absolute z-10 mt-1 w-64 bg-white rounded-lg shadow-lg border border-gray-200 py-1">
                  {getAvailableSubcategories().map(subcategory => (
                    <button
                      ...
                      className={`w-full px-4 py-2 text-sm text-left hover:bg-gray-50 transition-colors ${
                        activeSubcategory === subcategory.id ? 'bg-blue-50 text-blue-800' : 'text-gray-700'
                      }`}
                    >
                      {subcategory.name}
                    </button>
      ...
      <div className="space-y-12 mt-8">
        {Object.entries(displayProducts).map(([sectionName, { premium, regular }]) => (
          <div key={sectionName}>
            {/* Section Header */}
            <div className="flex items-center justify-between mb-6 pb-2 border-b border-gray-200">
              ...
              {premium.length + regular.length > 0 && (
                <span className="text-sm text-gray-500">
                  {premium.length + regular.length} {(premium.length + regular.length) === 1 ? 'product' : 'products'}
                </span>
              )}
            </div>
      ...
      {Object.keys(displayProducts).length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-700 mb-2">No products selected</h3>
          <p className="text-gray-500 mb-6">You haven't selected any products in this category yet.</p>
        </div>
      )}
