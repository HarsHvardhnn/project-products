...
import { Calendar, Clock, Video, Link as LinkIcon, Copy, Share2, Mail, Phone, X, AlertCircle, ChevronDown, Plus, User } from 'lucide-react';
...
export const MeetingScheduler: React.FC<MeetingSchedulerProps> = ({
  ...
  const [shareMethod, setShareMethod] = useState<'copy' | 'email' | 'sms' | null>(null);
  const [contactInfo, setContactInfo] = useState('');
  const [showContactSelector, setShowContactSelector] = useState(!contactName);
  const [selectedContact, setSelectedContact] = useState<Contact | null>(
    contactName ? { id: '0', name: contactName, email: contactEmail || '', phone: contactPhone || '', type: 'customer' } : null
  );
  ...
  const generateMeetingLink = () => {
    ...
    const link = `https://qwillo.com/meeting/${uniqueId}`;
    setMeetingLink(link);
  };
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-lg font-medium text-gray-900">
                {isInstantMeeting ? 'Create Instant Meeting' : 'Schedule Meeting'}
              </h3>
              {selectedContact && (
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex space-x-4">
                      <button
                        onClick={() => setNewContact(prev => ({ ...prev, type: 'customer' }))}
                      ...
                      <button
                        onClick={() => setNewContact(prev => ({ ...prev, type: 'lead' }))}
                        className={`flex-1 px-3 py-2 rounded-md text-sm font-medium ${
          ...
          {selectedContact && (
            <>
              <div className="mb-6 space-y-4">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Face-to-face meeting with screen sharing</p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-900">
                      {isInstantMeeting ? '5-Minute Meeting' : 'Duration: ' + duration + ' minutes'}
                    </p>
                    <p className="text-sm text-gray-500">
                      {isInstantMeeting ? 'Quick consultation to understand needs' : 'Select your preferred duration'}
                    </p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-900">
                      {isInstantMeeting ? 'Instant Availability' : 'Schedule for Later'}
                    </p>
                    <p className="text-sm text-gray-500">
                      {isInstantMeeting ? 'No scheduling required - start immediately' : 'Pick a convenient time'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              {!meetingLink ? (
                ...
                >
                  {isInstantMeeting ? 'Generate Meeting Link' : 'Schedule Meeting'}
                </button>
              ) : (
                <div className="space-y-4">
                      ...
                      <div className="ml-3">
                        ...
                        <div className="mt-2 text-sm text-yellow-700">
                          <ul className="list-disc pl-5 space-y-1">
                            ...
                            <li>Screen sharing is available for both parties</li>
                          </ul>
                        </div>
                      </div>
                    </div>
